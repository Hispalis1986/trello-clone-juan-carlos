...
// La función WATCH observa cualquier propiedad de un objeto en busca de cambios. 
// Esto también incluye la primera asignación de valor a la propiedad. 
// La función de observación incluye un controlador que tiene acceso al valor anterior y al valor nuevo de la propiedad observada del objeto.
watch(
    () => props.payload,
    (newValue) => {
...
const deleteItem = (type) => {
    if (type === 'container') {
        trello.containers = trello.containers.filter((container) => container.id !== state.selectedContainerId)
        trello.cards = trello.cards.filter((card) => card.id_container !== state.selectedCardId)
    } else {
        trello.cards = trello.cards.filter((card) => card.id !== state.selectedCardId)
    }
    trello.last_modified = new Date().toLocaleString('en-GB')
    store.dispatch('trello/setTrello', trello)
...
const handleKanbanAction = (mode, type, containerId, payload) => {
    ...
    if (mode === 'add') {
        if (type === 'container') {
            const newContainer = {
                id: trello.containers.length > 0 ? [...trello.containers].pop().id + 1 : 1,
                name: newContainerTitle.value,
            }
            trello.containers.push(newContainer)
            state.isAddingContainer = false
        } else {
            newCardData.id = trello.cards.length > 0 ? [...trello.cards].pop().id + 1 : 1
        ...
        if (type === 'container') {
            trello.containers = trello.containers.filter((container) => container.id !== containerId)
            state.isRemovingContainer = false
        }
        trello.cards = trello.cards.filter((card) => card.id_container !== containerId)
    } if (payload) {
        payload.is_editing_container ? (payload.is_editing_container = false) : (payload.is_adding_card = false)
    }
    trello.last_modified = new Date().toLocaleString('en-GB')
    store.dispatch('trello/setTrello', trello)
    newCardData.id = null
...
<template>
    <div
        class="flex h-full w-full overflow-auto rounded-lg bg-gradient-to-r md:bg-gradient-to-r from-blue-500 to-pink-500 px-2 py-3">
        <TransitionGroup name="list">
            <div v-for="container in trello.containers" :key="container.id" class="mx-1">
                <div class="min-h-[50px] min-w-[300px] max-w-[300px] rounded-lg bg-[#E4E5EC] p-1"
                    ...
                    <div class="flex flex-col overflow-y-auto" style="max-height: calc(100vh - 165px)">
                        <div v-for="card in cardList(container.id)" :key="card.id"
                            class="m-1 cursor-pointer rounded-lg bg-white p-2" :draggable="state.isDraggable"
                            @dragstart="dragItem($event, card)">
                            <Transition name="fade" mode="out-in">
                                <div v-if="card.is_editing_card"
                                    class="flex w-full flex-col rounded-md border-gray-700 bg-white">
                                    ...
                                    <div class="mt-2 flex w-full place-items-center justify-between">
                                        <Button model="outline" size="sm" rounder="sm"
                                            @click="handleDeleteItem('card', card.id)">
                                            Delete Card {{ index }}
                                        </Button>
                                        <div class="flex">
                                            <SaveIcon height="30px"
                                                class="mr-2 cursor-pointer rounded-full bg-yellow-500 p-1 text-white hover:bg-blue-700"
                                    ...
                                    <div class="flex flex-col" @click="handleEditCard('change', card)">
                                        <div class="flex place-items-center justify-between">
                                            ...
                                            <MoveIcon height="25px"
                                                class="cursor-grab rounded-full p-1 text-red-700 hover:bg-gray-200 hover:text-yellow-700"
                                                @click.stop="" @mouseenter="state.isDraggable = true"
                            ...
                            <Transition name="fade" mode="out-in">
                                <div v-if="container.is_adding_card"
                                    ...
                                    <SaveIcon height="30px"
                                        ...
                                        @click="handleKanbanAction('add', 'card', container.id, container)" />
                                    <CloseIcon height="30px"
                                        class="cursor-pointer rounded-full p-1 text-red-500 hover:bg-red-600 hover:text-white"
        ...
        <div class="mx-1">
            <div
                class="flex min-h-[50px] min-w-[300px] flex-col place-items-center justify-center rounded-lg bg-[#ebe4ec] p-2">
                <Transition name="fade">
                ...
                <Transition name="fade" mode="out-in">
                    ...
                    <Button v-else type="primary" model="outline" size="sm" rounded="sm"
    ...
    <ConfirmationModal :value="state.isRemovingCard" @confirm="deleteItem('card')"
